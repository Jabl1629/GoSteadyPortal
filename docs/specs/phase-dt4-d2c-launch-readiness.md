...
> **Date:** 2026-07-08 | **Status:** 🟢 all 4 workstreams IMPLEMENTED; backend + facility-frontend DEPLOYED to dev (portal `4289809` WS1 / `6dc62d2` WS2 / `318b0c6` WS4; WS3 Twilio secret populated + OTP proven live). **First browser test surfaced 3 gaps — see coord §C54:** (1) the live D2C app (`main_d2c.dart`) is **not hosted** (ran locally on `:8080` for the test); (2) the dashboard hit **`401`** — D2C Cognito JWT likely not accepted by the API authorizer (fix first); (3) email-verify + SMS-OTP is **double verification** (UX decision). Also owed: WS2 walker alert-rate check (pre-PROD), physical exit-bar test, deferred cosmetics (walkerId→claimId, D2C test device). **UPDATE (§C55, 2026-07-08):** gap (1) **401 FIXED** — D2C reads re-registered under `/api/v1/d2c/*` on the D2C-pool authorizer; deployed to `GoSteady-Dev-Api` + verified. gap (2) **hosting added** — `app.gosteady.co` (own `GoSteady-Dev-D2CHosting` stack + `deploy-d2c-app.sh`); deploy is **operator-DNS-gated** (ACM cert PENDING_VALIDATION). Both form factors **staged claimable** (`GS0001000041` walker / `GS0001000043` rollator) + WS1 rendering **locked with tests** (13/13). gap (3) email+SMS **decided: phone-only SMS-OTP** (drop the email-verify gate; D2C pool reconfig — alias vs new pool — is a scoped follow-up). Remaining: operator DNS→finish hosting, SMS browser E2E on the hosted URL for both form factors, WS2 alert-rate check.
