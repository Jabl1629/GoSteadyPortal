...
- **Status**: ✅ **Deployed (dev) 2026-05-25** — end-to-end smoke verified at https://dev.portal.gosteady.co/
...
|---|----------|--------|-----------------|--------|
| T1 | Live-mode sign-in (no MFA) | At https://dev.portal.gosteady.co/, sign in as `rd-caregiver@test.local` (caregiver role, no MFA) | ✅ LoginScreen → Cognito USER_PASSWORD_AUTH → `/dev/me` rendered the full claim set (`clientId: client_rd_test`, `role: caregiver`, `facilities: fac_rd_a`, `censuses: cen_rd_a1`, `internalAccess: false`, `mfaEnrolled: false`) — verified 2026-05-25 | ✅ **Pass** |
| T2 | Live-mode sign-in (MFA-required role) | Sign in as `rd-facadmin@test.local` (facility_admin) with TOTP enrolled via authenticator app; enter the 6-digit code at the MFA challenge | ✅ LoginScreen → password OK → SDK threw `CognitoUserTotpRequiredException` → caught → routed to `/mfa-verify` mode → enter TOTP → `completeMfaChallenge` → `/dev/me` rendered with `facility_admin` claims — verified 2026-05-25 | ✅ **Pass** |
...
| T11 | Both builds compile cross-version | Run `flutter build web` for both entries before any commit | ✅ Both succeed; verified during Phase H impl + multiple deploys since | ✅ **Pass** |
| T12 | CDK deploy of `GoSteady-Dev-Hosting` succeeds in one pass | Add validation CNAMEs at Squarespace per Option B pre-deploy; run `cdk deploy GoSteady-Dev-Hosting` | ✅ Stack reached CREATE_COMPLETE in ~27 min (one retry needed due to initial DNS-record-input error — see Changelog 2026-05-25); ACM cert validated; CloudFront distribution `Deployed`; all 5 outputs surfaced — verified 2026-05-25 | ✅ **Pass** |
| T13 | First portal deploy lands at `dev.portal.gosteady.co` | After T12 + Squarespace CNAME for `dev.portal`: run `./tools/deploy-portal.sh` | ✅ `https://dev.portal.gosteady.co/` returned HTTP/2 200 with Flutter bundle; cert valid (no browser warning); LoginScreen rendered — verified 2026-05-25 | ✅ **Pass** |
| T14 | API call from real domain works (CORS smoke) | Sign-in flow at `https://dev.portal.gosteady.co/dev/me` | ✅ Sign-in succeeded; `/dev/me` returned 200 with full claims; no CORS preflight failure — verified 2026-05-25 for both caregiver + facility_admin paths | ✅ **Pass** |
| T15 | SPA deep link works through CloudFront | Navigate to `/dev/me` from `/census` (deep link via GoRouter) | ✅ CloudFront SPA 404→index.html rewrite worked; Flutter routed correctly; MeSmokeScreen rendered — verified 2026-05-25 | ✅ **Pass** |
...
| T17 | Deploy invalidates CloudFront cache | Run `./tools/deploy-portal.sh`; check `index.html` timestamp before + after | ✅ Three deploys today (initial + USER_PASSWORD_AUTH fix + TOTP exception fix); each was visible within ~60 s of script completion — verified 2026-05-25 | ✅ **Pass** |
...
| D16 | **Sign-in uses `USER_PASSWORD_AUTH`, not SRP** (the SDK's default) | Stay on `USER_SRP_AUTH` (the SDK default) | The `amazon_cognito_identity_dart_2` SDK's SRP path uses Web Crypto API and was returning `NotAuthorizedException` on Flutter Web despite correct credentials — verified against the same user via `aws cognito-idp initiate-auth --auth-flow USER_PASSWORD_AUTH`, which succeeded. `USER_PASSWORD_AUTH` sends the password to Cognito over HTTPS; same approach AWS Amplify uses for browser-based Cognito apps. The Portal-Customer App Client has `ALLOW_USER_PASSWORD_AUTH` enabled per Phase 0A-rev — no infra change needed. **Discovered 2026-05-25 during T1 validation**; committed in `7d91c4b`. Switch is one line: `_cognitoUser!.setAuthenticationFlowType('USER_PASSWORD_AUTH')` in `auth_service.dart::signIn` |
| D17 | **Catch `CognitoUserTotpRequiredException` for TOTP MFA** (not just `CognitoUserMfaRequiredException`) | Catch only `CognitoUserMfaRequiredException` (the SMS variant the SDK throws) | The SDK throws `CognitoUserTotpRequiredException` (a different class) when Cognito returns ChallengeName=`SOFTWARE_TOKEN_MFA`; `CognitoUserMfaRequiredException` is reserved for the legacy `SMS_MFA` ChallengeName. Phase 0A-rev disabled SMS at the pool level, so SOFTWARE_TOKEN_MFA is the only path V1 will see. Both are caught defensively. **Discovered 2026-05-25 during T2 validation**; verified MFA challenge flow end-to-end with rd-facadmin@test.local + authenticator-app-generated TOTP |
...
| 2026-05-24 | Jace + Claude session | Pulled minimum-viable hosting into 2B-0 per user direction. Added `GoSteady-Dev-Hosting` CDK stack (S3 + CloudFront + OAC + ACM + baseline WAF), `tools/deploy-portal.sh`, `dev.portal.gosteady.co` URL, Squarespace DNS lock-ins (L16 / L17), operator-action prereqs for ACM cert validation, T12–T17 acceptance scenarios, decisions D11–D15 |
| 2026-05-24 | Jace + Claude session | Implementation landed across 3 commits: `128428d` (spec drafting), `3ad87ae` (Flutter foundation — auth/api/data/state/shell/smoke; 34 files, 2144 insertions), `0d63ec5` (hosting stack + deploy script). FacilityRepository interface deviation noted: kept sync to avoid ~25-site Future-cascade through demo screens; live impl in 2B-FAC-R will handle async via cache + refresh discipline |
| 2026-05-25 | Jace + Claude session | **Deployed end-to-end.** CDK hosting stack live (one rollback retry — first deploy got placeholder text in the DNS validation record by mistake); fresh deploy issued same hash, real CNAME added; ACM validated in ~5 min; CloudFront propagated in ~22 min. Merge with `feature/infra-scaffold` to pick up parallel 2A-UM-P + 1C-slim work (one tiny ARCHITECTURE.md conflict — clean resolve). API stack deployed with new CORS allow-list. First portal deploy succeeded. T1 + T13–T15 + T17 ✅ Pass via `rd-caregiver@test.local`. T2 ✅ Pass via `rd-facadmin@test.local` with authenticator-app TOTP. Two SDK-related fix commits added: `7d91c4b` (USER_PASSWORD_AUTH switch — D16) + this commit (TOTP exception catch — D17) |
