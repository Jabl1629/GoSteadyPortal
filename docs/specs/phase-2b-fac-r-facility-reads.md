...
| 2026-05-26 | Jace + Claude session | **CR-1 deploy + live validation closed end-to-end (coord §C28.5).** `cdk deploy GoSteady-Dev-Processing` succeeded in ~41 s; first post-deploy heartbeat at 16:42:23Z populated `Device Registry.lastSeen` correctly. Diagnosed that the live S3 portal bundle was from before `89bd76f` (timestamps showed S3 upload at 21:36:31 vs commit at 21:36:52 the same day) — `flutter clean && ./tools/deploy-portal.sh` with `AWS_REGION=us-east-1` exported republished a fresh 4.4 MB `main.dart.js` + CloudFront-invalidated. Live portal now shows "Last seen: 1 min ago", Today's Activity 8m/198ft/356 steps, and Census list-view's Active-min/Steps/trend columns populate after `rowStatsFor` resolves. Two tooling gotchas recorded in §C28.5.5 (deploy script needs `AWS_REGION`; `flutter clean` required after stale-build deploys). No new commits — only docs landed |
| 2026-05-26 | Jace + Claude session | **L3 + L11 PollingController landed + live-validated.** New `lib/state/polling_controller.dart` — `WidgetsBindingObserver`-backed singleton mounted at `AppShell` root, exposed via `AppState.polling`. Census subscribes via `didChangeDependencies` and re-fetches `/me/patients` + per-row stats every 60 s; Patient Detail subscribes for the active `patientId` and re-fetches `/patients/{id}` + activity + alerts every 30 s. Both timers pause when `AppLifecycleState != resumed` (per L11) and resume immediately on `resumed`. Live-validated against `pt_bench_98`: network capture confirms `/me/patients` poll fires at the 60 s mark and `/patients/{id}` Future.wait fan-out fires every 30 s while Patient Detail is open. Foreground-only behavior + tab-blur pause satisfies umbrella A4 audit-cost ceiling. Remaining 2B-FAC-R follow-ups: lazy-per-row throttle (L5; mostly invisible at 1-patient bench scale), notification engine swap (L6; live screens still display "—" badge text), 30s TTL caching (L12; would dedupe duplicate `?range=30d` fan-out fetches on each detail tick) |
| 2026-05-26 | Jace + Claude session | **L6 notification engine swap to server alertType landed + live-validated.** New `notificationsFor(patientId)` repository method (replaces the engine wrapper at the call sites); live impl reads cached `/alerts?status=unacknowledged` and maps each `alertType` to `NotificationType` via the spec's L6 table (`no_activity_today`, `below_typical_activity`, `declining_trend`, `device_offline`, `device_silent`, `battery_critical/low`, `signal_lost/weak`, plus `other` catch-all). Demo impl unchanged — still delegates to `NotificationEngine` against `notificationContextFor`. Census list-view's Notifications cell now shows the **rule-name** for the highest-severity unack alert (with "· N" suffix when N>1) instead of just a count; Patient Detail's Notification Review panel renders each alert with its mapped label + relative-time `detail`. Verified against `pt_bench_98` real-data: Census badge reads "Battery critical · 45" (red severity dot beside resident name), Patient Detail panel shows "NOTIFICATIONS · 45 AWAITING REVIEW" header + per-alert rows with "Triggered 38 min ago / 1h ago / 2h ago". `notification_engine.dart`'s global `notificationsForPatient(data, id)` helper is retained as a one-line delegator so existing screen call sites don't need to chase the rename. Demo build (`BUILD_MODE=demo`) compiles cleanly + still uses the engine. Remaining 2B-FAC-R follow-ups: L5 lazy-per-row throttle (mostly invisible at 1-patient bench), L12 30s TTL caching + in-flight dedup |
| 2026-05-26 | Jace + Claude session | **L12 TTL caching + in-flight Future dedup.** Refactored `LiveFacilityRepository`'s five per-patient `Map<String, T>` caches (`_patientDetailCache`, `_activity24hCache`, `_activity7dCache`, `_activity30dCache`, `_alertsCache`) to use a new `_TimedCache<K, V>` helper at file scope. Each cache entry carries a `fetchedAt` timestamp; a 30 s TTL is checked on read. A parallel `Map<K, Future<V>> _inFlight` deduplicates concurrent fetches for the same key — when Patient Detail's `_load()` calls `_fetchActivity(d30)` twice in parallel (once via `last30DaysFor`, once via `last6MonthsFor`), both calls share one HTTP request instead of racing. `refreshPatientDetail` now calls `evict(patientId)` per cache (in-flight fetches are not cancelled; the next caller after the evict misses the cache and starts a fresh fetch — natural eventual consistency). `clearOnSignOut` calls `clear()` on each cache. Cache writes go through a single code path so the TTL-window guarantees coord doc §C29.2's "30 s consistency window" — every consumer of the same data within 30 s sees the same value. `flutter analyze` clean; demo build (`BUILD_MODE=demo`) compiles unchanged (mock impl doesn't touch the cache). Bench validation against pt_bench_98 ran into Chrome MCP layout-shift artifacts during the multi-step click sequence; trust-the-code based on standard async pattern, clean analyzer, and the explicit poll-tick refresh path that evicts caches anyway. **All five 2B-FAC-R follow-ups documented as remaining-after-2B-FAC-R-initial-slice are now complete**, except L5 (lazy-per-row throttle, mostly invisible at single-patient bench scale; deferred until 50+ patient seed exists) |
| 2026-05-26 | Jace + Claude session | **L5 row-loader throttle landed.** New `lib/state/row_loader_queue.dart` — generic `RowLoaderQueue<T>` capped at 5 concurrent tasks with per-key in-flight dedup (re-enqueueing the same `patientId` while a prior task is pending returns the existing Future). `LiveFacilityRepository`'s per-patient stats fetches go through the queue from `PatientCensusView`, capping the cold-load fan-out so a 200-patient facility doesn't burst past dev API Gateway's 25 RPS throttle. New `lib/widgets/maybe_visible.dart` (`MaybeVisible`) wraps each list/tile row in `visibility_detector` so future visibility-lazy work just needs to flip the eager-enqueue path off; the package dependency landed (`visibility_detector: ^0.4.0+2`). Skeleton placeholder: sage-tinted `_SkeletonBar` rendered in each metric cell while a row is loading (kept static, not pulsing — keeps the Census quiet during cold-load). Visibility-lazy callbacks proved fragile in release Flutter Web (callbacks intermittently didn't fire post-service-worker handover), so the Census now eager-enqueues all visible rows on init through the throttle. Wrapping the wire-up still uses `MaybeVisible` defensively — if the visibility callback DOES fire, `_scheduleLoad` short-circuits because the row is already loaded. Live-validated against `pt_bench_98`: skeleton bars render on cold load, resolve to real data after the throttled fetch completes ("Battery critical · 45" notification badge, Active min today=1, 7d avg=9, trend ↗, Steps today=35, trend ↗). Demo build (`BUILD_MODE=demo`) compiles unchanged. **2B-FAC-R now substantively complete for V1 critical-path; visibility-lazy is the one remaining sub-item to revisit at multi-patient pilot scale** |
