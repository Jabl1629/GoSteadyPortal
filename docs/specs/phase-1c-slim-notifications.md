...
| L5 | Idempotency: per-rule conditional PutItem using compound SK `{facility-local midnight, with offset}#{alertType}` for the daily-cadence rules; `{localNow}#{alertType}` for offline rules (which dedupe via the open-alert state machine instead). Reruns within the same local day are no-ops | This spec | Pattern from 1B-rev Threshold Detector P4 (conditional PutItem with attribute_not_exists guard). **Shipped 2026-05-24 with a second-precision `localNow` SK on the daily rules, which could never collide — so they had NO write-layer dedupe and their daily-ness rested entirely on the hour gate. Corrected 2026-07-27** (`history_window.local_day_anchor_iso`); see Changelog |
...
| L8 | "No activity today" rule: triggered at facility-local **11:00** (env `NO_ACTIVITY_LOCAL_HOUR`), plus a catch-up window of `TRIGGER_CATCHUP_HOURS` (default 3 → hours 11–13). Fires alert if `sum(primary metric) over [local-midnight, local-now] == 0` AND `lastSeen < 24h` (device is alive but not moving) | This spec — Q3 decided; hour raised 09:00 → 11:00 on 2026-07-27 | Demo's behavior triggers immediately; production needs a buffer. 09:00 still flagged residents who had merely had a slow morning — 11:00 lets breakfast *and* mid-morning activity land first, trading ~2h of notice for materially fewer false CRITICALs |
...
|---|---|---|---|---|
| No activity today | `no_activity_today` | critical | Facility-local 11:00 (+ catch-up to 13:00) | Activity Series query over [local-midnight, local-now]; Device Registry `lastSeen` (must be < 24h to fire) |
...
behavioral-detector Lambda
   ...
   │            local 11..13   → no_activity_today rule
   │            local 22..23   → below_typical + declining_trend rules
...
| `NO_ACTIVITY_LOCAL_HOUR` | 11 | 11 | L8 (was 9 until 2026-07-27) |
| `END_OF_DAY_LOCAL_HOUR` | 22 | 22 | L9 / L10 |
| `TRIGGER_CATCHUP_HOURS` | 3 | 3 | Hourly firings, from the target hour, in which a daily rule may still run. Clamped at local midnight. Set to `1` to restore exact-hour-equality behavior |
...
| D6 | "No activity today" rule triggers at local-11, not local-00 (L8) | Trigger at midnight; local-09 (the original choice) | A 06:00 alarm for a patient who simply hasn't woken up yet is noise. 09:00 proved to still be inside "slow morning" territory; 11:00 (2026-07-27) lets breakfast and mid-morning activity land first |
| D11 | Daily-rule SK anchors to facility-local **midnight**, not local-now (L5) | Keep local-now-to-the-second | A second-precision SK can never collide, so the conditional PutItem was dead code for these rules and daily-ness depended solely on the hour gate firing exactly once. The day anchor makes the write itself the once-per-day guard — and is what lets D12's window be safe |
| D12 | Daily rules run on a **window** (target hour + `TRIGGER_CATCHUP_HOURS`), not exact-hour equality | Exact `local_hour == target`; a boundary-crossing test against the previous firing | Exact equality silently dropped a facility's whole day whenever an invocation was missed or two firings straddled the hour boundary (`list_facilities` raising takes out every facility at once). With D11's day anchor the extra firings cost one Activity query each and write nothing. DST needs no special handling: US transitions move hours 01–03, so an hourly cron still sees local 11 and 22 exactly once on a transition day (verified by simulation) |
...
### Q3. "No activity today" trigger hour — hard-coded or per-facility configurable?
**Decision (2026-07-27):** ✅ Env-configurable, environment-wide — `NO_ACTIVITY_LOCAL_HOUR`, now **11**. Raised from 09:00 because 09:00 still flagged residents who had merely had a slow morning. Per-*facility* override remains V2 (some memory care facilities have earlier breakfast), via the same threshold-override pattern as 2A-AA.
...
| Q3 | No-activity trigger hour | ✅ local-11, env-tunable; per-facility override still V2 |
...
| Date | Author | Change |
|------|--------|--------|
| 2026-07-27 | Jace + Claude (portal session) | **Daily-cadence correctness pass + trigger hour raised to 11:00.** Three changes, dev+prod pending. (1) **SK day anchor (L5 / new D11).** The daily rules stamped `eventTimestamp` — and so their Alert History SK — with facility-local now *to the second*, which can never collide across two invocations. The `ConditionExpression` in `_write_alert` was therefore dead code for them (its own comment claimed "same-day dedupe"), and once-per-day rested entirely on the hour gate firing exactly once. New `history_window.local_day_anchor_iso()` anchors them to facility-local midnight; 8 unit tests incl. both DST directions. Offline rules keep local-now (they dedupe via the open-alert state machine). Fixes the weakness ai-coach C2-D4 called out. (2) **Trigger window (new D12).** `rule_set_for_facility` was `local_hour == target`, so any missed or jitter-straddled firing silently dropped that facility's entire day — and `list_facilities` raising drops *every* facility at once. Now a window: target hour + `TRIGGER_CATCHUP_HOURS` (default 3), clamped at local midnight so the end-of-day rules can't leak into the next day's anchor. Safe only because of (1). **Note the DST framing in the original docstring was wrong** — US transitions move local hours 01–03, so an hourly UTC cron always saw local 09/11/22 exactly once even on transition days; simulation confirms. 22 new tests in a new `test_facility_iterator.py` (the gate previously had zero coverage), including a counterfactual that reproduces the old miss at `catchup_hours=1`. (3) **`NO_ACTIVITY_LOCAL_HOUR` 9 → 11** (L8 / D6 / Q3) — 09:00 flagged residents who had merely had a slow morning. (4) **Portal follow-on:** a midnight-anchored `eventTimestamp` would have made a fresh 11:00 alert render "Triggered 11h ago", so `createdAt` is now projected by `patient-api._alert_view` and `AlertRow.raisedAt` (new, falls back to `eventTimestamp`) is what both the facility and D2C repos age against. 76 behavioral + 59 patient-api + 29 alert-actions + 104 `_shared` green; `dart analyze` clean; `cdk synth` confirms the new env vars. |
