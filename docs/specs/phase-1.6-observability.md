...
| 2026-05-05 | Jace + Claude (cloud session) | **Audit log + dashboard query follow-up** (commit `ec258f5`). Per-Device Detail dashboard's "Recent activity sessions" widget was rendering only the `steps` column despite DDB rows being fully populated. Two chained cloud-side bugs surfaced as soon as activity uplinks started flowing reliably from real-walk data (firmware fix `4d62fd5` resolved a stop_done_sem race that had been silently dropping activity uplinks for ≥2 weeks): (1) `activity-processor`'s `emit_audit` `after` block only included sessionEnd/steps/activeMinutes/date — not the firmware-derived optional fields that the dashboard query expected; (2) the dashboard Logs Insights query used firmware-side snake_case field names (`after.distance_ft`, `after.roughness_R`, etc.) but the audit log uses camelCase matching DDB column names. Both fixed: handler.py emits the full `after` block when fields are present (accept-all D16 contract), and `per-device.ts` query aliases the camelCase audit-log fields to the snake_case dashboard column names. Verified end-to-end against five real walks (35/40/10/81/60 steps with distance + R + surface_class + firmware_version all rendering after a hard-refresh of the browser tab). Operational note added: CloudWatch dashboards don't auto-invalidate client cache on widget schema changes; operators need to hard-reload after any redeploy that changes column structure. |
