...
| **2B-FAC-R** Facility Reads | 🔲 Planned ([`phase-2b-fac-r-facility-reads.md`](phase-2b-fac-r-facility-reads.md)) | Wires `LiveFacilityRepository` to 2A-RD endpoints — Census, Patient Detail, Device Detail, Notification badges (using real 1C-slim alertType names). Polling cadence 60s/30s with foreground-only pause. List view default per user-needs US-04. 6M tab + gait UI suppressed in V1. Demo build continues to work unchanged via approach C |
