...
| **Ops surface** — review queue (trial), transcript export for daily review, kill-switch config flag | CLI-grade, matches "Support: ad-hoc (Slack + CLI)" posture |
...
```
   ...
   │ → conditional PutItem inbox (1/day) → [trial: review queue]            │
...
| Q2 | **Cohort & consent** — ✅ **Direction (2026-07-18):** coach disclosures go into a **general user agreement shown at walker setup** (first-run), with the coach disclaimers called out clearly and acknowledged — see **Appendix A** (draft copy). Open sub-parts: (a) on-by-default-with-off-switch vs a gating opt-in checkbox; (b) family-Admin households (where the Member isn't the walker user) | Coach on for all trial account-holders, disclosed + acknowledged in the setup agreement (lean: prominent acknowledgment, not a friction checkbox); family-Admin households deferred |
...
| Q4 | **Escalation protocol** — when triage flags self-harm/emergency/abuse: beyond the scripted 988/911 response, who is notified? (a) nobody (resources only), (b) GoSteady ops (= Jace, during trial), (c) care-circle Admin — and does the user consent to (c) up front? | (b) during trial via existing ops alarm path; (c) is a product decision needing explicit user consent — design the consent, decide later |
...
| Q6 | **Memory visibility** — user-editable (decided, D4) — but can care-circle members see coach conversations or memory? | No — coach is private to the walker user; family sees only "coach is active." Revisit only with user consent framing |
| Q7 | **Daily-message channel** — portal-inbox-only until 10DLC lands, or block C2 on SMS? Quiet hours / frequency prefs default? | Ship portal-only, add SMS teaser when Phase-2 registration clears; default 1/day max, off-switch in prefs |
| Q8 | **Retention** — coach transcripts + memory TTL (activity is 13 mo, alerts 24 mo); LLM-side: Bedrock retains nothing by default — confirm no cross-border inference (us geo profile) is acceptable | 24 mo transcript TTL, memory lives until user deletes / account closes; us geo profile |
| Q9 | **Voice timing & modality** — is voice in the trial at all, or post-trial? Browser widget vs phone-call check-in (Meela pattern — we hold every user's verified phone number) | Post-trial C4; seriously evaluate **phone-call** as the primary voice modality for this demographic |
| Q10 | **Success metrics** — define before C1: engagement (% opening coach ≥3×/wk), morning-message open rate, activity delta (steps/active-min vs pre-coach baseline), safety (flagged-turn rate, zero critical incidents), qualitative delight | Instrument all four from day one via audit events |
| Q11 | **Counsel review** — one-time review of SB 243 / NY Art. 47 applicability + the published crisis-protocol page + consent copy | Yes, before real users; cheap insurance given private right of action |
| Q12 | **Facility-tier future** — does the coach concept extend to facility residents (staff-supervised)? Doesn't change V1, but validates the Bedrock/BAA platform choice | Park it; D1 keeps the door open |
...
| D6 | Deterministic triggers + LLM copywrite + review queue | Recommended |
...
| Q1–Q12 | Scoping decisions | **Open — this session** |

---

## Appendix A — Draft onboarding disclosure copy (coach)

> **Draft for review, 2026-07-18.** Plain language, ~6th-grade reading level, large-type friendly. This is the **coach-specific block** inside the broader general user agreement shown at walker setup (first-run onboarding).
>
