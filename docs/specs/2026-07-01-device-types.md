...
|---|---|---|
| Activity required metrics | `steps`, `distance_ft`, `active_min` | Bench v0: `active_min` only → **walker-cap parity at DT-2 exit** (`steps`, `distance_ft`, `active_min` required) per D10 |
| Activity optional metrics | `roughness_R`, `surface_class`, `gait_speed_fts`, `firmware_version` | `gait_speed_fts` at parity (D10); rollator-specific extras emerge during the algo arc |
