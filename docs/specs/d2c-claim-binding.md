...
> **Date:** 2026-07-13 (v2 same day) · shipped dev + prod 2026-07-14 · canon pass 2026-07-14 | **Status:** 🟢 SHIPPED (dev + prod) — §5.1–5.11 live; ARCHITECTURE canon reconciled (§10). Only remaining: a real-Cognito-token E2E of the claim leg
...
- [x] **Canon supersession pass — DONE 2026-07-14.** Superseded the `dtc_{userId}`/`{primaryUserId}`/`{sub}` anchor → `dtc_{householdId}` in ARCHITECTURE §4 + §14 T3, d2c.md L1, phase-0b-revision L6, d2c-phone-only-signin §7 #3 (marked cut-over), and a superseded banner on d2c-phase1-walker-activation; documented the per-claim `RoleAssignments`/`clientId` rewrite (ARCHITECTURE §4 + T3). Added **`release`** as the third ownership-mutation class (DL4/DL9) and the new events (`device.ownership_released`, `device.claim_bound`, `device.claim_binding_cleared`, `d2c.claim_rejected_phone_mismatch`, `d2c.claim_rejected_device_owned`, `device.wipe_reissued`) to the §4 audit catalog. Rescoped DL5 "no pre-allocation" to the facility channel. Updated §6 Devices (walkerId + `by-walker-id` GSI + `claimBoundPhone`/`Mask` + lifecycle attrs; fixed the stale status enum) and §15 (device-api 14 routes + `d2c-claim` + D2C-Auth trigger Lambdas).
...
| 2026-07-14 (prod) | **Deployed to prod** — GoSteady-Prod-Api + Prod-Processing (D2C-Auth was a 0-diff no-op) + both frontends (`app.gosteady.co`, `portal.gosteady.co`). Prod smoke 5/5 (admin_create → bind → `reserved` lookup through prod API GW, synthetic device cleaned up) + screenshot of the live prod `reserved` landing. Pilot device GS0002000001 confirmed untouched (additive change). Remaining: real-Cognito-token E2E of the claim leg; canon-supersession doc pass (§10). |
| 2026-07-14 (phone fix) | Fixed a shared phone-normalization bug (frontend + backend both trusted a stray `+`): "+5165891580" (US number missing +1) was read as country code +516 → Twilio rejected the OTP (21408). US-shaped numbers now normalize to `+1…` regardless of a stray `+`; 5 new `test_claim_binding_normalize` cases. Also moved the reserved-flow phone entry to the /setup landing (last-4-validated against the mask, carried via router `extra`) so the account page is **name + optional email only** — no phone field to diverge from the bound number. |
| 2026-07-14 (canon) | **Canon-supersession pass (§10) done.** Reconciled ARCHITECTURE.md + sibling specs to the shipped D2C model: `dtc_{householdId}` anchor (§4, §14 T3, d2c.md, phase-0b-rev, phone-only-signin, phase-1 banner) + the per-claim pointer rewrite; `release` as the third ownership-mutation class (DL4/DL9) + the 6 new audit events (§4); DL5 rescoped to the facility channel; §6 Devices table (walkerId + `by-walker-id` GSI + binding fields + corrected 5-state status enum); §15 (`device-api` 14 routes + `d2c-claim` + D2C-Auth Lambdas). |
