...
|---|---|---|---|
| **2B-0** Foundation | ✅ **Deployed (dev) 2026-05-25** ([phase-2b-0-foundation.md](phase-2b-0-foundation.md)) | `ApiClient`, real Cognito auth in Flutter (sign-in / sign-out / token refresh / session restore), full-claim `GoSteadyUser`, JWT-attached HTTP client, error-envelope decoder, retry + backoff, network-failure UX, MFA challenge handler, forgot-password flow. `FacilityRepository` abstraction + dual-build (`BUILD_MODE=demo\|live`) so the same screen tree powers both the marketing demo and the live portal. **One smoke screen** — calls `GET /api/v1/me` and displays the claims (parallels 2A-0's stub-endpoint smoke approach). **Plus minimum-viable hosting**: `GoSteady-Dev-Hosting` CDK stack live at `dev.portal.gosteady.co`. T1 / T2 / T11–T15 / T17 ✅ Pass (caregiver + MFA-required facility_admin both verified end-to-end) | 2A-0 (deployed) |
| **2B-FAC-R** Facility Reads | 🔲 Planned ([phase-2b-fac-r-facility-reads.md](phase-2b-fac-r-facility-reads.md)) | Login → Facility Shell → Census (list + tile) → Patient Detail (overlay) → Device Detail. Pure-read swap of the demo's `FacilityMockData` for `LiveFacilityRepository` against 2A-RD endpoints. Notification badges driven by `/patients/{id}/alerts` with full rule-name copy from 1C-slim alertTypes (no longer count-only — A3 gate closed by 1C-slim deploy). Polling cadence: 60s Census + 30s Patient Detail while foreground. List view is V1 default (per user-needs US-04). 6M tab + gait UI both suppressed in live mode pending firmware + Phase 1C-full | 2A-RD + 1C-slim (both deployed) + 2A-0 |
...
|---|-------------|-----------|-----------|
| L1 | **One portal, one URL, one Cognito App Client** (no client secret). All users — `caregiver` / `facility_admin` / `client_admin` / `household_owner` / `family_viewer` / `internal_support` / `internal_admin` — sign in at the same URL (`portal.gosteady.co` in prod). Role-specific UI affordances (internal cross-tenant search, audit-reader nav) appear at runtime based on the `custom:role` JWT claim. The Portal-Internal Cognito client (`gvc7n839vj4ppgioamknlk21c`, has secret) is **retired from browser use** and reserved for non-browser internal tools (CLI, server-side admin scripts). **Amended 2026-05-24:** "one Flutter build" wording removed — the portal codebase produces TWO build artifacts (`BUILD_MODE=live` → `portal.gosteady.co`; `BUILD_MODE=demo` → `facilitydemo.gosteady.co` marketing demo) from a single screen tree per [phase-2b-0-foundation.md](phase-2b-0-foundation.md) L1+D1. The portal is still one URL; the demo is a separate marketing artifact, not "a second portal" | This spec — Q8 decided 2026-05-23; dual-build clarification 2026-05-24 | The security boundary is the JWT claim, not the URL. Pre-Token Lambda enforces MFA + claim injection + tenancy regardless of which client authenticated, and API Gateway 403s any caller whose role doesn't authorize the endpoint — so a unified-build customer never gets data they shouldn't, regardless of whether they discovered an internal endpoint name by inspecting the JS bundle. Matches [ARCHITECTURE.md](ARCHITECTURE.md) §4's stated stance ("internal access is allowed but always visible to compliance") rather than the obscurity-layer the dual-client approach baked in. **Requires a 2A-0 follow-up amendment**: the JWT authorizer's `audience` list narrows from two clients to one (see §Open Questions Q8 + implementation note) |
...
Swaps the demo's `FacilityMockData` for `LiveFacilityRepository` **on a per-screen basis under the live build**. Per 2B-0 ([phase-2b-0-foundation.md](phase-2b-0-foundation.md) L1 + D1), the demo's screens, widgets, and state classes stay in `lib/facility_demo/` and continue to power the marketing demo at `facilitydemo.gosteady.co` via the `BUILD_MODE=demo` build. `FacilityMockData` is a first-class data source for the demo build, NOT a test fixture. 2B-FAC-R adds `LiveFacilityRepository` (already stub-introduced in 2B-0) as the parallel `FacilityRepository` impl that the live build wires up. Screens are unchanged — both impls satisfy the same interface.
...
| `lib/data/facility_repository.dart` | 2B-0 | New | Abstract `FacilityRepository` interface (see [phase-2b-0-foundation.md](phase-2b-0-foundation.md) §Interfaces) |
| `lib/data/live_facility_repository.dart` | 2B-0 / 2B-FAC-R | New | `ApiClient`-backed `FacilityRepository`. Stubs land in 2B-0; per-screen wiring fills in during 2B-FAC-R |
| `lib/facility_demo/data/facility_mock_data.dart` | 2B-0 | Modified | Add `implements FacilityRepository`; promote sync methods to `Future<...>`. **Stays in `lib/facility_demo/`** as the canonical demo data source (NOT moved to test_support) |
