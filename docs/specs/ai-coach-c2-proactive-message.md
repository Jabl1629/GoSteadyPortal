...
| C2-D2 | **Add a third hour-gate knob `COACH_LOCAL_HOUR` (default 13)** and an `evaluate_coach = in_trigger_window(local_hour, coach_hour, catchup)` branch in the `rule_set_for_facility` shape. Window + hourly cron + C2-D4's date-bucketed SK = the once-per-day cadence. | Mirrors `NO_ACTIVITY_LOCAL_HOUR=11` / `END_OF_DAY_LOCAL_HOUR=22` (`facility_iterator.py`; CDK `processing-stack.ts`). **Use the window helper, not `local_hour == coach_hour`** — the exact-hour form silently dropped a whole day whenever a firing was missed or straddled the hour boundary (phase-1c-slim D12, fixed 2026-07-27). ~1:30 pm lands within the 1 pm local hour (gate is hour-precision). |
...
| C2-D4 | **Robust ≤1/day via a date-bucketed SK:** inbox SK = `INBOX#<facility-local YYYY-MM-DD>` (the SK C1 §5.1 reserved), conditional PutItem `attribute_not_exists`. This is ≤1/day independent of sub-hour re-fires (retries, DST fallback repeating the 1 pm hour). | Backend audit §4 flagged that the detector's daily-ness relied on the hour-gate + a second-precision SK; date-bucketing makes it robust. Reuses the identical `ConditionExpression`. **Update 2026-07-27:** behavioral-detector's daily rules now anchor their SK to facility-local midnight as well (phase-1c-slim D11), so the two approaches are equivalently robust — this is no longer a point of difference. |
