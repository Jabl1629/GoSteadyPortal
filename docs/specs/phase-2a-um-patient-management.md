...
- **Status**: ✅ Deployed (dev) 2026-05-24 — 27/27 synthetic smoke PASS; end-to-end audit pipeline verified
- **Branch**: `feature/infra-scaffold`
- **Date Started**: 2026-05-24
- **Date Completed**: 2026-05-24 (dev)
...
| 2026-05-24 | Jace + Claude (portal session) | **Deployed to dev** across 5 commits on `feature/infra-scaffold` (commits `a02f52b` scaffold + pure-fn → `f9cc17a` handler + inline atomic provision → `73727b1` pause-aware threshold-detector + activity-processor + patient-api response → `71b5354` CDK wiring → `e5ea6e6` smoke runner + deploy summary). **Synthetic smoke 27/27 PASS** covering: create (no device / family_viewer denied / bad census / empty name / malformed serial / caregiver out-of-scope); PATCH (single-field happy / multi-field / empty body 400 / cross-facility caregiver-denied + client_admin-allowed); pause + GET-reflects-pause + invalid-reason 400 / days=91 400; resume + double-resume 409; care-note set + GET-reflects / over-280 400 / empty-clears; family_viewer pause denied; discharge happy + double-discharge 409 + PATCH-on-discharged 409; no-token 401. **End-to-end audit pipeline verified**: 10 audit events from smoke landed in `gosteady-dev-audit` log group within ~3s, all tagged `schema_version: 1` + auto-stamped `internal_access` + `severity`, with full `before`/`after` maps on every PATCH (including cross-facility transfer logging facilityId change explicitly). **Deploy chronology**: hit two ordering quirks — (a) cross-stack-export ordering (Api stack newly consumed Data's UsersTable export; deployed Data first to publish then Api succeeded); (b) synthetic invoke to materialize the patient-mgmt log group before the Audit-stack filter attached (same as 2A-RD deploy). Total deploy time ~3 min across Processing + Data + Api + Audit stacks. **One design pivot from spec**: atomic create-with-device chain switched from Lambda-invoke (spec L3 recommendation) to inline duplication of device-api's provision logic (~80 lines mirrored in `patient-mgmt/handler.py::_provision_inline`). Rationale: avoids forged-API-Gateway-event awkwardness + doubled cold start + extra IAM. Trade-off: drift risk if device-api's provision logic changes. TODO marker in code points at `_shared/provision.py` as the refactor target if/when both diverge. Three follow-ups filed: atomic-add-with-device path needs bench-session validation (needs a fresh `ready_to_provision` device — synthetic smoke validates the handler shape but not the live IoT publish); `suppressed_paused` audit sample rate needs Patient-row dedupe to hit ≤1/day/patient target (currently ≤1/invocation/patient = ~24/day under hourly cron); the inline-provision-vs-shared-module decision documented inline. Coord §C27 captures the full deploy chronology. |
