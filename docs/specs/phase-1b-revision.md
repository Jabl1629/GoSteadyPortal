...
| 2026-05-25 | Jace + Claude (portal session) | **heartbeat-processor `lastSeen` mirror** (coord §C28.1 / 2B-FAC-R CR-1): every heartbeat now also writes `lastSeen` to the Device Registry row (in addition to Shadow.reported.lastSeen). Without this write, the registry row's lastSeen stayed null forever (Shadow was the only writer) and patient-api's `lastSeen or firstHeartbeatAt` fallback returned stale provisioning timestamps for every live device. Best-effort write (non-fatal on failure; Shadow remains authoritative). No IAM change — existing `dynamodb:UpdateItem` grant covers it. Deploy via `cdk deploy GoSteady-Dev-Processing --context env=dev`. |
