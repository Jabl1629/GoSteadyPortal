...
Only the Logger / Metrics / Tracer interfaces touched by
`_shared/observability.py` need stubs; everything below is the bare
minimum to import that module without ImportError.
"""
...
def _install_stub() -> None:
    ...
    def _noop_or_decorator(*a, **kw):
        """Callable stub that works BOTH as a plain no-op call AND as a
        decorator factory (`@logger.inject_lambda_context(...)` /
        `@metrics.log_metrics(...)` must return something that, applied to
        a function, returns that function)."""
        if len(a) == 1 and callable(a[0]) and not kw:
            return a[0]  # used directly as @decorator
        def _decorate(fn):
            return fn
        return _decorate
    ...
    class _NoopLogger:
        ...
        # explicit methods commonly called
