...
class HourlyActivity {
  ...
  /// Average gait speed in ft/s during walking bouts in this hour.
  /// Zero when the user did not walk this hour.
  final double avgGaitSpeedFts;

  /// Slowest sustained walking-bout speed observed in this hour, ft/s.
  final double minGaitSpeedFts;

  /// Fastest sustained walking-bout speed observed in this hour, ft/s.
  final double maxGaitSpeedFts;
  ...
  const HourlyActivity({
    ...
    this.avgGaitSpeedFts = 0,
    this.minGaitSpeedFts = 0,
    this.maxGaitSpeedFts = 0,
  });
}
...
class DailyActivity {
  ...
  double get minGaitSpeedFts {
    final active = hours.where((h) => h.minGaitSpeedFts > 0);
    if (active.isEmpty) return 0;
    return active.map((h) => h.minGaitSpeedFts).reduce((a, b) => a < b ? a : b);
  }

  /// Fastest sustained speed across the day's active hours.
  double get maxGaitSpeedFts {
    final active = hours.where((h) => h.maxGaitSpeedFts > 0);
    if (active.isEmpty) return 0;
    return active.map((h) => h.maxGaitSpeedFts).reduce((a, b) => a > b ? a : b);
  }
}
...
class WeeklyActivity {
  ...
  /// Time-weighted average gait speed (ft/s) across the week.
  final double avgGaitSpeedFts;

  /// Slowest sustained gait speed observed in the week (ft/s).
  final double minGaitSpeedFts;

  /// Fastest sustained gait speed observed in the week (ft/s).
  final double maxGaitSpeedFts;
